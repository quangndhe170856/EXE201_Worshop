...
export function filterEvents(events: Event[], filters: FilterState): Event[] {
  ...
  filteredEvents = filteredEvents.filter(event =>
    event.price.min >= minPrice && event.price.max <= maxPrice
  )
...
export function sortEvents(events: Event[], sortBy: string): Event[] {
  ...
  switch (sortBy) {
    ...
    case 'price-low':
      return sortedEvents.sort((a, b) => a.price.min - b.price.min)

    case 'price-high':
      return sortedEvents.sort((a, b) => b.price.max - a.price.max)
